...
import com.jwatson.omnigame.Sprite;
 ...
 * 
 * @author mzechner */
public class CustomBatch implements Disposable {
	private Mesh mesh;
	...
	/** Constructs a new CustomBatch. Sets the projection matrix to an orthographic projection with y-axis point upwards, x-axis
	...
	/** Constructs a CustomBatch with the specified size and (if GL2) the default shader. See
	 * {@link #CustomBatch(int, ShaderProgram)}. */
	public CustomBatch (int size) {
		this(size, null);
	}
	 ...
	 * @param defaultShader the default shader to use. This is not owned by the CustomBatch and must be disposed separately. */
	public CustomBatch (int size, ShaderProgram defaultShader) {
		this(size, 1, defaultShader);
	}

	/** Constructs a CustomBatch with the specified size and number of buffers and (if GL2) the default shader. See
	 * {@link #CustomBatch(int, int, ShaderProgram)}. */
	public CustomBatch (int size, int buffers) {
	 ...
	 * @param defaultShader the default shader to use. This is not owned by the CustomBatch and must be disposed separately. */
	public CustomBatch (int size, int buffers, ShaderProgram defaultShader) {
		...
		for (int i = 0; i < buffers; i++) {
			this.buffers[i] = new Mesh(VertexDataType.VertexArray, false, size * 4, size * 6, new VertexAttribute(Usage.Position, 2,
				...
				new VertexAttribute(Usage.TextureCoordinates, 2, ShaderProgram.TEXCOORD_ATTRIBUTE + "0"),new VertexAttribute(Usage.TextureCoordinates, 2, ShaderProgram.TEXCOORD_ATTRIBUTE + "1"),new VertexAttribute(Usage.Normal, 1, "a_brightness"));
		}

		projectionMatrix.setToOrtho2D(0, 0, Gdx.graphics.getWidth(), Gdx.graphics.getHeight());

		vertices = new float[size * (Sprite.SPRITE_SIZE2)];
	...
	/** Returns a new instance of the default shader used by CustomBatch for GL2 when no shader is specified. */
	...
	public void begin () {
		if (drawing) throw new IllegalStateException("you have to call CustomBatch.end() first");
		renderCalls = 0;
	 ...
	 * {@link #begin()} */
	public void end () {
		if (!drawing) throw new IllegalStateException("CustomBatch.begin must be called before end.");
		if (idx > 0) renderMesh();
		lastTexture = null;
		idx = 0;
		drawing = false;
	...
	/** Sets the color used to tint images when they are added to the CustomBatch. Default is {@link Color#WHITE}. */
	...
	/** @return the rendering color of this CustomBatch. Manipulating the returned instance has no effect. */
	...
	public void draw (TextureRegion region, float x, float y) {
		draw(region,1, x, y, region.getRegionWidth(), region.getRegionHeight());
	}

	/** Draws a rectangle with the bottom left corner at x,y and stretching the region to cover the given width and height. */
	public void draw (TextureRegion region,float brightness, float x, float y, float width, float height) {
		if (!drawing) throw new IllegalStateException("SpriteBatch.begin must be called before draw.");
		...
		//brightness = Terrain.CurrentTerrain.GetLightTile((int)x, (int)y)/15f;
	...
	public void draw (TextureRegion region,TextureRegion region2,float brightness, float x, float y, float width, float height) {
		if (!drawing) throw new IllegalStateException("SpriteBatch.begin must be called before draw.");
		...
		final float u_2 = region2.getU();
		final float v_2 = region2.getV2();
		final float u2_2 = region2.getU2();
		final float v2_2 = region2.getV();
