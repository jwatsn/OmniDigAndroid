...
//				TerrainChunk ch = Terrain.CurrentTerrain.GetChunkByID(x, y);
		...
		for(TerrainChunk ch : Terrain.CurrentTerrain.ActiveChunks) {
			culling.set(ch.x*Terrain.chunkWidth+Terrain.chunkWidth/2,ch.y*Terrain.chunkHeight+Terrain.chunkHeight/2, 0);
...
//				
					...
					for(int x2=0; x2<Terrain.CurrentTerrain.chunkWidth; x2++) {
						for(int y2 = 0; y2<Terrain.CurrentTerrain.chunkHeight; y2++) {
							...
							byte mybright = GetBakedBrightness(ch.x*Terrain.chunkWidth + x2,ch.y*Terrain.chunkHeight + y2);
							
							byte bright_right = GetBakedBrightness((ch.x*Terrain.chunkWidth + x2)+1,ch.y*Terrain.chunkHeight + y2);
							byte bright_left = GetBakedBrightness((ch.x*Terrain.chunkWidth + x2)-1,ch.y*Terrain.chunkHeight + y2);
							byte bright_up = GetBakedBrightness((ch.x*Terrain.chunkWidth + x2),(ch.y*Terrain.chunkHeight + y2)+1);
							byte bright_down = GetBakedBrightness((ch.x*Terrain.chunkWidth + x2),(ch.y*Terrain.chunkHeight + y2)-1);
...
//								floodStack.push(new recursiveLight(ch.x, ch.y,x2,y2-1,(byte)15));
//								floodStack.push(new recursiveLight(ch.x, ch.y,x2-1,y2,(byte)15));
								recursiveLight2((ch.x*Terrain.chunkWidth + x2),(ch.y*Terrain.chunkHeight + y2),15);
							}
	...
	boolean SetLightMap(int x3, int y3, int value) {
		...
		TerrainChunk chk = Terrain.TerrainChunks[x+y*Terrain.Width];
		if(chk != null) {
		chk.LightMap2[x2+y2*Terrain.chunkWidth] = new Byte((byte) value);
...
//		 if(y%Terrain.chunkHeight == 31 && y/Terrain.chunkHeight != (int)(Bob.CurrentBob.pos.y/Terrain.chunkHeight)) {
//			 Gdx.app.debug(""+y/Terrain.chunkWidth + " " + (int)(Bob.CurrentBob.pos.y/Terrain.chunkHeight), ""+tileBrightness);
//		 }
